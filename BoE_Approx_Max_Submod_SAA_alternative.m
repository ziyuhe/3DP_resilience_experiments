...
if input.auto_recompute == 1
    ...
    elseif input.recompute_distr == 2
        ...
        if sum(x) < 5
            ...
            %% For |A_t|<5, we can still apply full info GRB
            input.recompute_flag = 1;
            input.recompute_sample_mode = 3;
            input.recompute_demand_sample_flag = 0;
    
        else
    
            %% For |A_t|>12, we apply SGD and for the final evaluation 
            input.recompute_flag = 2;
            input.recompute_sample_mode = 3;
            input.recompute_demand_sample_flag = 1;
            input.recompute_demand_samplesize_eval = ceil(2^12*100/2); 
            input.recompute_demand_samplesize_finaleval = ceil(2^12*100/2); 
    
        end

    elseif input.recompute_distr == 3

        %% Interpolate ind. + comono. disruptions
        if sum(x) < 5

            %% For |A_t|<5, we can still apply full info GRB
            input.recompute_flag = 1;
            input.recompute_sample_mode = 5;
            input.recompute_demand_sample_flag = 0;
            input.recompute_disruption_sample_flag = 0;
...
elseif input.recompute_flag == 2
    ...
    if input.recompute_distr == 1
        input_sgd.disrupt_distr = 1;
    elseif input.recompute_distr == 2
        input_sgd.disrupt_distr = 2;
    elseif input.recompute_distr == 3
        input_sgd.disrupt_distr = 3;
        input_sgd.p0 = input.p0;
    end

    output_sgd = U3DP_SGD_fixed_suppselect_fixed_K_alternative(input_sgd);
