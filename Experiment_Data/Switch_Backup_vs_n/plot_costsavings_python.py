...
def percent_formatter(x, _):
    return f"{x:.0f}%"  # Format as 'value%' without decimals

# Define num_suppliers_set
num_suppliers_set = [8, 15, 30, 45]
data_files = [f"Data_NumSuppliers_costsavings_{num_suppliers}.csv" for num_suppliers in num_suppliers_set[1:]]
...
plt.ylabel("Cost Savings (%)", fontsize=19)

plt.grid(True, color='gray', linestyle='--', linewidth=0.5, alpha=0.7)
plt.savefig("Cost_savings_boxplots.pdf")
plt.show()
