...
synth_features <- read.csv(paste0(filename_base, "_synth_features.csv"))
...
# 1. Find unique values and sort them (largest to smallest)
unique_profits <- sort(unique(raw_features$X3DP.profit), decreasing = TRUE)
# 2. Create a function to find position in unique values
get_position <- function(x) {
  which(unique_profits == x)
}
# 3. Create new feature by applying the function to each profit value
profit_positions <- sapply(raw_features$X3DP.profit, get_position)
# 4. Add this as a new feature
raw_features$profit_rank <- profit_positions
synth_features$profit_rank <- profit_positions
# Verify additions:
head(raw_features)
head(synth_features)
...
new_names <- c("3DP Cost-to-Profit", "Mean Shortfall")
...
# Let user select features (example)
# You can modify these vectors to select different features

# Try everything all together
# selected_raw_features <- c("v", "h", "c3DP", "cTM", "weight", "X3DP.profit", "demand.mean", "disrupt..prob.", "yield.loss")  # example selection
# selected_synth_features <- c("Abs.3DP.profit","3DP Cost-to-Profit","service.level","Mean Shortfall" )  # example selection

# Try raw features
# selected_raw_features <- c("h", "cTM", "X3DP.profit", "demand.mean", "disrupt..prob.", "yield.loss", "profit_rank")
# selected_synth_features <- c()

# Try synthetic features
selected_raw_features <- c()
selected_synth_features <- c("3DP Cost-to-Profit", "service.level","Mean Shortfall")


# Combine selected features
X <- cbind(
  raw_features[, selected_raw_features, drop=FALSE],
...
chosen_splits <- 2  # example value
chosen_depth <- 3   # example value
...
pdf("final_decision_tree.pdf", width = 8, height = 6)  # Set PDF dimensions
...
opt_cp <- final_tree$cptable[which.min(final_tree$cptable[,"xerror"]), "CP"]

# Prune the tree using the optimal cp
pruned_tree <- prune(final_tree, cp=0.03)
